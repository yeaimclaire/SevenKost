	...
	public function index()
	{
	    ...
	    $data['email'] = $this->session->userdata('email');
		     ...
		     else
		     {
		        ...
		        if($id_role == '1')
		        {
		          header('location:'.base_url().'Dashboard_Admin/dashboard');
		        }
		        else if($id_role == '2'){
		          header('location:'.base_url().'Main_Back_User');
		      	}
				  else if($id_role == '3'){
					header('location:'.base_url().'Main_Back_Owner/dashboard');
				}
				else if($id_role == '4'){
					header('location:'.base_url().'Main_Back_Superadmin/dashboard');
				}
	    ...
	    if ($this->form_validation->run()==FALSE) {// jika validasi terjadi kesalahan maka akan kembali ke halaman awal
	      ...
	      if ($hitung_datadb>0) {
	        ...
	        if($this->session->userdata('id_role')=='1') {
					redirect(base_url('Dashboard_Admin/dashboard'));
	        //redirect('C_Front/login_administrator');
			  }elseif ($this->session->userdata('id_role')=='2') {
					redirect(base_url('Main_Back_User'));
	        ...
	        //redirect('C_Front/login_hima');
					...
					redirect(base_url('Main_Back_Superadmin/dashboard'));
	        //redirect('C_Front/login_hima');
				}
